...
/*full read and write functions for fortran by liuqimin 2011*/

void read_data_(const char *f1, SACHEAD *hd, float *dt)
{
	float *da=0;
	da=read_sac(f1,hd);
...
int	read_sachead_(const char *f1,  SACHEAD *hd)
{
	int st=read_sachead(f1,hd);
}
int	write_sac_(const char *f1,  SACHEAD *hd, const float *da)
{
	write_sac(f1,*hd,da);
